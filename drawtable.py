...
class Table:
    def __init__(self,
        ...
        for value in (row_sep_char, headers_row_sep_char,
                      corner_char, cell_sep_char, cell_fill_char):
            if len(value) != 1:
                raise SimpleTableError("Table structure parameters must have length 1: '{}'".format(value))

        self.min_h_padding = max(self.min_h_padding, 0)

        if self.newline not in SUPPORTED_NEWLINES:
            raise SimpleTableError("newline '{}' not supported".format(newline))
    ...
    def draw(self):
        ...
        self.header_row_separator = self._build_row_sep(row_sep_char=self.header_row_sep_char)

        return ('{row_sep}{newline}'
                '{header}{newline}'
                '{header_sep}{newline}'
                '{data_rows}').format(row_sep=self.row_separator,
                                      header=self._build_row(self.headers),
                                      header_sep=self.header_row_separator,
                                      data_rows=self.newline.join([
                                          self._build_row(row) +
                                          self.newline +
                                          self.row_separator for row in self.data]),
    ...
    def _get_list_of_lists(self, data):
        ...
        return [[row_dict.get(column_key, self.default_value)
    ...
    def _get_column_widths(self):
        ...
        for col_index, header in enumerate(self.headers):
            column_widths[col_index] = max(column_widths[col_index],
                                           len(header) + self.min_h_padding * 2)
    ...
    def _build_row_sep(self, row_sep_char=None):
        ...
        return '{}{}{}'.format(self.corner_char,
                               self.corner_char.join([row_sep_char * min_col_length
                                                      for min_col_length in self.column_widths]),
                               self.corner_char)
    ...
    def _build_row(self, row):
        ...
        for line_index in range(row_height):    # for each line
            ...
            for column_index in range(len(row)):
                ...
                line.append(self._fill_h_cell_padding(value, self.column_widths[column_index]))    # cell padding

            lines.append('{}{}{}'.format(self.cell_sep_char,  # first |
...
headers = ["Group and cardinality", "Element", "Description", "Data type and cardinality"]
table_data = [
    ["| Issuer identification\n| 1..1", "Country", "Country of the issuer", "| Coding (ISO 3166-1 alpha-2)\n| 1..1"],
    ["| Issuer identification\n| 1..1", "Name", "Name of the issuer", "| String\n| 1..1"],
    ["| Issuer identification\n| 1..1", "Identifier", "Identifier of the issuer", "| Identifier (format to be defined later)\n| 1..1"],
    ["| Issuer identification\n| 1..1", "Public key or PKI certificate", "Public key or PKI certificate assigned to the issuer", "| Text (format to be defined later)\n| 1..*"],
    ["| Issuer identification\n| 1..1", "Online verification webapp", "Address of the online verification webapp, if offered by the issuer", "| URI\n| 0..1"],
    ["| Issuer identification\n| 1..1", "Online verification endpoint", "Address of the online verification service, if offered by the issuer", "| URI\n| 0..1"],

    ["| Issuer authorization\n| 1..*", "Health certificate type", "The type of health certificates the issuer is capable and authorised to issue", "| Coding (value set: vaccination certificate)\n| 1..1"],
    ["| Issuer authorization\n| 1..*", "Validity from", "Start of the authorisation period", "| dateTime\n| 1..1"],
    ["| Issuer authorization\n| 1..*", "Validity to", "End of the authorisation period", "| dateTime\n| 1..1"],
    ["| Issuer authorization\n| 1..*", "Status", "Status of the authorisation", "| Coding (active, inactive, entered-in-error, on-hold, unknown) https://www.hl7.org/fhir/valueset-account-status.html\n| 1..1"],
]
